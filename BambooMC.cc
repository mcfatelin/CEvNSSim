...
int main(int argc, char *argv[]) {
    ...
    if (BambooGlobalVariables::Instance()->getPhysicsName().find("Physics") ==
        ...
        auto physicsList =
            BambooPhysicsFactory::create(
                BambooGlobalVariables::Instance()->getPhysicsName(),
                BambooGlobalVariables::Instance()->getPhysicsName());
        runManager->SetUserInitialization((G4VModularPhysicsList *)physicsList.release());
    }

    auto generator =
        BambooGeneratorFactory::create(
            BambooGlobalVariables::Instance()->getGeneratorName(),
            BambooGlobalVariables::Instance()->getGeneratorName());

    runManager->SetUserAction((G4VUserPrimaryGeneratorAction *)generator.release());

    // analysis
    auto analysis =
        BambooAnalysisFactory::create(
