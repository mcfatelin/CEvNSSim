...
const Int_t QS1RawNest::_MAX_DECAYTIME = 500; // ns

TRandom3 QS1RawNest::_rr(0);
TRandom3 QS1RawNest::_ra(0);
TRandom3 QS1RawNest::_rz(0);
TRandom3 QS1RawNest::_rd(0);
TRandom3 QS1RawNest::_r(0);
TRandom3 QS1RawNest::_ra1(0);
TRandom3 QS1RawNest::_ra2(0);
// --------------------------------------------------------------------
// --------------------------------------------------------------------
// --------------------------------------------------------------------
// --------------------------------------------------------------------
...
std::vector<G4ThreeVector> QS1RawNest::GenUniformDirection(Int_t np)
{
	//TRandom3 rd(0);
...
G4ThreeVector QS1RawNest::GenAbsolutePosition(const Double_t rmax, const Double_t hmin, const Double_t hmax)
{
	...
	return G4ThreeVector(std::sqrt(r)*std::cos(angle), 
						 std::sqrt(r)*std::sin(angle), 
						 _rz.Rndm()*(hmax-hmin) + hmin);
}

G4ThreeVector QS1RawNest::GenAbsolutePosition2(const Double_t rmin, const Double_t rmax, const Double_t hmin, const Double_t hmax)
{
	//TRandom3 rr(0), ra(0), rz(0);

	const Double_t r = _rr.Rndm()*(rmax*rmax-rmin*rmin)+rmin*rmin;
	const Double_t angle = _ra.Rndm()*2*M_PI;
...
G4ThreeVector QS1RawNest::GenPointAbsolutePosition(const Double_t rmax, const Double_t x, const Double_t y, const Double_t z)
{
	const Double_t r = _r.Rndm() * rmax;
	const Double_t angle1 = _ra.Rndm() * M_PI;
	const Double_t angle2 = _ra.Rndm() * 2 * M_PI;

	return G4ThreeVector(
		x + r * std::sin(angle1) * std::cos(angle2),
		y + r * std::sin(angle1) * std::sin(angle2),
		z + r * std::cos(angle1));
}

G4ThreeVector QS1RawNest::GenCylinderAbsolutePosition(const Double_t rmax, const Double_t x, const Double_t y, const Double_t z, const Double_t h)
{
	const Double_t dz = _rz.Rndm() * h - 0.5 * h;
	const Double_t r = _r.Gaus(0,rmax);//standard gaus random distribution for r direction
	const Double_t angle = _ra.Rndm() * 2 * M_PI;
...
std::vector<G4int> QS1RawNest::GenTimingPhoton(const TH1I* const h)
{
	vector<G4int> t;
